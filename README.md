...
4. скаченные веса распаковать в корневую директорию проекта
5. запустить файл `get_submission.py` с указанием аргументов (директории, где лежат аудиофайлы, и директории, где будет сохранён файл submission.json) путём запуска следующей команды
